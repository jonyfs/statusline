...
test("skills expire once they stop being used", async () => {
  const { getActiveSkills } = await import("../src/skills.js");
...
test("skills survive out-of-order and timestamp-less transcript entries", async () => {
  ...
  const { getActiveSkills } = await import("../src/skills.js");
  const { writeFileSync, mkdtempSync } = await import("node:fs");
  const file = path.join(mkdtempSync(path.join(os.tmpdir(), "skills-")), "t.jsonl");
  ...
  writeFileSync(
    file,
    [
      use("2026-01-01T12:00:00.000Z", "wanted"),
      JSON.stringify({ type: "bridge-session" }),
      use("2026-01-01T09:00:00.000Z", "stray-old-entry"),
      use(null, "no-timestamp"),
    ].join("\n")
  );
